...
impl Graph {
    pub fn delete_selection(self: &mut Self, ctx: &mut Context) {
        if ctx.get_selected_links().len() > 0 {
            let links_to_remove: Vec<(usize, usize)> = ctx
                .get_selected_links()
                .iter()
                .map(|i| self.links[*i])
                .collect();
            self.links = self
                .links
                .iter()
                .filter(|val| !links_to_remove.contains(val))
                .map(|val| (val.0, val.1))
                .collect();
        }

        for node_idx in ctx.get_selected_nodes().iter() {
            for attr_idx in self.nodes[node_idx]
                .attributes
                .iter()
                .map(|attr| attr.attr_idx)
            {
                self.links = self
                    .links
                    .iter()
                    .filter(|val| (val.0 != attr_idx) && (val.1 != attr_idx))
                    .map(|val| (val.0, val.1))
                    .collect();
            }
...
impl Graph {
    pub fn update(self: &mut Self, ctx: &mut Context, ui: &mut egui::Ui) {
        ...
        for (_id, node) in self.nodes.iter() {
            let mut gnode = NodeConstructor::new(node.node_idx, NodeArgs::default());
            ...
            for atr in node.attributes.iter() {
                match atr.attr_type {
                    NodeAttrType::STATIC => {
                        gnode = gnode
                            .with_static_attribute(atr.attr_idx, |ui| ui.label(atr.text.as_str()));
                    }
                    NodeAttrType::INPUT => {
