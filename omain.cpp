...
int amplification( void *outputBuffer, void *inputBuffer, unsigned int /*nBufferFrames*/,
           double streamTime, RtAudioStreamStatus status, void *data )
{
  ...
  for (int i = 0; i < bytes / sizeof(MY_TYPE); ++i){
    ((MY_TYPE*)outputBuffer)[i] = (((MY_TYPE*)inputBuffer)[i]) * 4;
  }
  return 0;
}
...
int main( int argc, char *argv[] )
{
  ...
  if ( adac.openStream( &oParams, &iParams, FORMAT, fs, &bufferFrames, &amplification, (void *)&bufferBytes, &options ) ) {
    goto cleanup;
  }
